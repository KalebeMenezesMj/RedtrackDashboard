...
function mapRows(fbRows: FBInsightRow[]): AdRow[] {
  ...
  for (const row of fbRows) {
    ...
    const purchases = actionVal(row.actions, 'purchase') || actionVal(row.actions, 'omni_purchase') || actionVal(row.actions, 'offsite_conversion.fb_pixel_purchase')
    out.push({ name, playRate: plays / impressions, hookRetention: p25 / plays, bodyConversion: purchases / plays, bodyRetention: p75 / plays })
  }
  return out
}
